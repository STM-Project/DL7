...
unsigned char MODBUS_TimerInit(uint32_t period)
{
	htim5.Instance = TIM5;
	htim5.Init.Period = period;
	htim5.Init.Prescaler = (uint32_t) ((SystemCoreClock / 2) / 100000) - 1;
	htim5.Init.CounterMode = TIM_COUNTERMODE_UP;
	htim5.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
    return 1;
}

void MODBUS_TimersEnable(void)
{
...
void MODBUS_TimersDisable(void)
{
	 HAL_TIM_Base_Stop_IT(&htim5);
	 HAL_TIM_Base_DeInit(&htim5);
}

void MODBUS_TimerHandler(void)
{
