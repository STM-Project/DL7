...
#define LCD_FRAME_BUFFER ((U32)&Framebuffers[0])
...
static void _ClearCacheHook(U32 LayerMask) {
 ...
 if (LayerMask & (1 << i)) {
 SCB_CleanDCache_by_Addr ((uint32_t *)_aAddr[i],XSIZE_PHYS * YSIZE_PHYS * sizeof(U32) * NUM_BUFFERS/* * NUM_VSCREENS * NUM_BUFFERS*/);
 }
 }
}
...
void HAL_LTDC_LineEvenCallback(LTDC_HandleTypeDef *hltdc) {
  ...
  for (i = 0; i < GUI_NUM_LAYERS; i++)
  {
    if (_aPendingBuffer[i] >= 0)
    {
      ...
      Addr = (U32 )_aAddr[i] + _axSize[i] * _aySize[i] * _aPendingBuffer[i] * _aBytesPerPixels[i];
...
#if (osCMSIS <= 0x20000U)
  LcdUpdateSemaphoreId = osSemaphoreCreate(osSemaphore( LcdUpdateSemaphore ), 1);//  LcdUpdateSemaphoreId = osSemaphoreCreate( osSemaphore( LcdUpdateSemaphore ), 1 );
  osSemaphoreWait( LcdUpdateSemaphoreId , 1000 );
#else
	  LcdUpdateSemaphoreId = osSemaphoreNew( 1, 1, osSemaphore( LcdUpdateSemaphore ));//  LcdUpdateSemaphoreId = osSemaphoreCreate( osSemaphore( LcdUpdateSemaphore ), 1 );
	  osSemaphoreAcquire( LcdUpdateSemaphoreId , 1000 );
#endif
