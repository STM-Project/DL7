...
/* USER CODE BEGIN ETH link Thread core code for User BSP */
	    HAL_ETH_ReadPHYRegister(&heth, LAN8720A_PHY_ADDRESS_PHY_ADDRESS, PHY_BSR, &regvalue);

	    if(netif_is_link_up(netif) && !(regvalue & PHY_LINKED_STATUS))
	    {
	    	HAL_ETH_Stop_IT(&heth);
	      netif_set_down(netif);
	      netif_set_link_down(netif);
	    ...
	    else if(!netif_is_link_up(netif) && (regvalue & PHY_LINKED_STATUS))
	    {
